...
#include "../algo-origin/kronmult/kgemm/kgemm_nt_batched.hpp"
...
template<typename T>
T test_kgemm_nt_batched(int const mm, int const nn, int const kk, int const batchCount, int const idebug = 1)
{
    // ----------------------------------------
...
#ifdef USE_GPU
    {
        ...
        kgemm_nt_batched<T><<<batchCount, nthreads>>>(mm, nn, kk, alpha, ddAarray_, dldAarray_, ddBarray_,
...
#else
    {
        kgemm_nt_batched<T>(mm, nn, kk, alpha, ddAarray_, dldAarray_, ddBarray_, dldBarray_, beta, ddCarray_,
    ...
    if (idebug >= 1)
    {
        ...
        {
            std::cout << " mm = " << mm << " nn = " << nn << " kk = " << kk << " batchCount = " << batchCount
                      << "\n";
            std::cout << " elapsed time is " << elapsed_time_in_sec << " seconds " << gflops_per_sec
                      << " Gflops/s"
                      << "\n";
        };
    };
                ...
                for (int mm = 1; mm <= mm_max; mm += inc)
                {
                    ...
                    if ((!isok) || (idebug >= 2))
                    {
                        std::cout << " mm = " << mm << " nn = " << nn << " kk = " << kk
                                  << " batchCount = " << batchCount << " max_abserr = " << max_abserr << "\n";
                    };
                };
            };
        };
