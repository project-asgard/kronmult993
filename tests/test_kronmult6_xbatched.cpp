...
#include "../algo-origin/kronmult/xbatched/kronmult1_xbatched.hpp"
#include "../algo-origin/kronmult/xbatched/kronmult2_xbatched.hpp"
#include "../algo-origin/kronmult/xbatched/kronmult3_xbatched.hpp"
#include "../algo-origin/kronmult/xbatched/kronmult4_xbatched.hpp"
#include "../algo-origin/kronmult/xbatched/kronmult5_xbatched.hpp"
#include "../algo-origin/kronmult/xbatched/kronmult6_xbatched.hpp"
...
template<typename T>
T test_kronmult_xbatched(int const idim, int const n, int const batchCount, int const idebug = 0,
                         bool const do_check = true, bool const use_overlap_in_Y = true)
{
    int const lda = n + 3;
    ...
    auto dAarray = [&](int const i, int const j, int const k, int const ibatch) -> T & {
        return (dAarray_[indx4f(i, j, k, ibatch, lda, n, idim)]);
    };

    auto Aarray = [&](int const i, int const j, int const k, int const ibatch) -> T & {
...
#pragma omp parallel for
...
#ifdef USE_GPU
    {
        ...
        {
        case 1:
            kronmult1_xbatched<T>
                <<<batchCount, nthreads>>>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 2:
            kronmult2_xbatched<T>
                <<<batchCount, nthreads>>>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 3:
            kronmult3_xbatched<T>
                <<<batchCount, nthreads>>>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 4:
            kronmult4_xbatched<T>
                <<<batchCount, nthreads>>>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 5:
            kronmult5_xbatched<T>
                <<<batchCount, nthreads>>>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 6:
            kronmult6_xbatched<T>
                <<<batchCount, nthreads>>>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        default:
            assert(false);
...

    {
        ...
        {
        case 1:
            kronmult1_xbatched<T>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 2:
            kronmult2_xbatched<T>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 3:
            kronmult3_xbatched<T>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 4:
            kronmult4_xbatched<T>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 5:
            kronmult5_xbatched<T>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        case 6:
            kronmult6_xbatched<T>(n, dAparray_, lda, dpdZarray_, dpdYarray_, dpdWarray_, batchCount);
            break;
        default:
            assert(false);
        };
...

    {
        ...
        if (flops > 0.01 * giga)
        {
            std::cout << " idim = " << idim << " n = " << n << " batchCount = " << batchCount
...
#pragma omp parallel for collapse(6) reduction(max : max_abserr)
                    ...
                    for (int i3 = 1; i3 <= max_i3; i3++)
                        for (int i4 = 1; i4 <= max_i4; i4++)
                            for (int i5 = 1; i5 <= max_i5; i5++)
                                ...
                                {
                                    int const ic = 1 + indx6f(i6, i5, i4, i3, i2, i1, max_i6, max_i5, max_i4,
                                                              max_i3, max_i2);
                                    T Y_ic       = 0;
        ...
        for (int ic = 1; ic <= max_ic; ic++)
        {
            ...
            if (idebug >= 1)
            {
                ...
                if (abs_err > tol)
                {
                    std::cout << " idim = " << idim << " ic = " << ic << " Y_ic = " << Y_ic
                              << " Yval =  " << Yval << " abs_err = " << abs_err << "\n";
                };
            };
