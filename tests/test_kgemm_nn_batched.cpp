...
template<typename T>
T test_kgemm_nn_batched(int const mm, int const nn, int const kk, int const batchCount, int const idebug = 1)
{
    // ----------------------------------------
...
#ifdef USE_GPU
    {
        ...
        kgemm_nn_batched<T><<<batchCount, nthreads>>>(mm, nn, kk, alpha, ddAarray_, dldAarray_, ddBarray_,
...
#else
    {
        kgemm_nn_batched<T>(mm, nn, kk, alpha, ddAarray_, dldAarray_, ddBarray_, dldBarray_, beta, ddCarray_,
