...
#include "../no_batch/kronmult5.hpp"
...
kronmult5_batched(int const n, T const Aarray_[], T X_[], T Y_[], T W_[], int const batchCount)
//
...
//
{
    ...
    int const n4 = n2 * n2;
    int const n5 = n * n4;
    auto X = [&](int const i, int const j) -> T & { return (X_[indx2f(i, j, n5)]); };
    auto Y = [&](int const i, int const j) -> T & { return (Y_[indx2f(i, j, n5)]); };
    auto W = [&](int const i, int const j) -> T & { return (W_[indx2f(i, j, n5)]); };
