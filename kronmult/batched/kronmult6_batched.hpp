...
#include "../no_batch/kronmult6.hpp"
...
kronmult6_batched(int const n, T const Aarray_[], T X_[], T Y_[], T W_[], int const batchCount)
//
...
//
{
    ...
    auto X = [&](int const i, int const j) -> T & { return (X_[indx2f(i, j, n6)]); };

    auto Y = [&](int const i, int const j) -> T & { return (Y_[indx2f(i, j, n6)]); };

    auto W = [&](int const i, int const j) -> T & { return (W_[indx2f(i, j, n6)]); };
